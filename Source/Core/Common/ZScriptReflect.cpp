...
const std::map<Unique, ZScriptReflect::Type>& ZScriptReflect::GetGlobalMap() {
	...
	if (!inited) {
		...
		Declare<Void*>::Register("void*", globalReflectParserMap);
		Declare<Void>::Register("void", globalReflectParserMap);
		inited = true;
	}
...
void ZScriptReflect::ProcessProperty(IReflectObject& s, Unique typeID, const char* name, void* base, void* ptr, const MetaChainBase* meta) {
	if (read) {
		request >> key(name);
	} else {
		request << key(name);
...
void ZScriptReflect::ProcessMethod(Unique typeID, const char* name, const TProxy<>* p, const IReflect::Param& retValue, const std::vector<IReflect::Param>& params, const MetaChainBase* meta) {}
