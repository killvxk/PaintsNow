...
void CScriptEngine::Request::Debug(std::vector<StackInfo>& stackInfo, std::vector<LocalInfo>& localInfo, const Ref* func) {
...
IScript::Request& CScriptEngine::Request::operator << (const LocalInfo& k) {
	assert(host.GetLockCount() != 0);
	assert(key.empty());
	return *this;
}
...
IScript::Request& CScriptEngine::Request::operator >> (String& str) {
	...
	Read(*this, ptr);
	if (ptr != NULL) {
		_LeavesApi->WriteString(str, SystemToUtf8(String((const char*)ptr, ::SysStringByteLen(ptr))));
	}
...
IScript::Request& CScriptEngine::Request::operator << (const BaseDelegate& value) {
	...
	if (p != host.proxyPool.end()) {
		...
		proxy->SetProxy(reinterpret_cast<LONGLONG>(value.ptr));
		proxy->SetHost(reinterpret_cast<LONGLONG>(&host));
...
IScript::Request& CScriptEngine::Request::operator >> (BaseDelegate& value) {
	...
	if (dispatch != NULL) {
		...
		if (SUCCEEDED(dispatch->QueryInterface(__uuidof(IDelegateProxy), (void**)&proxy))) {
			LONGLONG base;
			proxy->GetProxy(&base);
			value.ptr = (reinterpret_cast<IScript::Object*>(base));
			proxy->Release();
		}
	}
	return *this;
...
void CScriptEngine::Request::DebugPrint() {}
...
bool CScriptEngine::Request::IsValid(const BaseDelegate& d) {
	return d.ptr != NULL;
}
...
IScript::Request::Ref CScriptEngine::Request::ReferenceWithDebugInfo(const BaseDelegate& d, const String& tag, int line) {
	assert(host.GetLockCount() != 0);
	// should use delegate!
	std::map<IScript::Object*, IDispatch*>::const_iterator p = host.proxyPool.find(d.ptr);
	if (p != host.proxyPool.end()) {
		p->second->AddRef();
		return IScript::Request::Ref((long)p->second);
	} else {
		return IScript::Request::Ref();
	}
}

IScript::Request::Ref CScriptEngine::Request::ReferenceWithDebugInfo(const Ref& d, const String& tag, int line) {
	assert(host.GetLockCount() != 0);
