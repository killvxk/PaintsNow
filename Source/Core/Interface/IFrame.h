...
namespace PaintsNow {
	...
	public:
		struct EventMouse {
			EventMouse(bool d, bool m, bool l, bool w, const ZInt2& p);
			bool down;
			bool move;
			bool left;
			bool wheel;
		...
		public:
			...
			virtual void OnWindowSize(const ZInt2& newSize) = 0;
			virtual bool IsRendering() const = 0;
		};

		virtual ~IFrame();
		virtual void SetCallback(Callback* callback) = 0;
		virtual const ZInt2& GetWindowSize() const = 0;
		virtual void SetWindowSize(const ZInt2& size) = 0;
		virtual void SetWindowTitle(const String& title) = 0;
		enum CURSOR { NONE, ARROW, CROSS, WAIT };
		virtual void ShowCursor(CURSOR cursor) = 0;
		virtual void WarpCursor(const ZInt2& position) = 0;
