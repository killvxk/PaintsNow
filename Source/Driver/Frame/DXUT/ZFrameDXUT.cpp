...
ZFrameDXUT::ZFrameDXUT(const ZInt2& size, IFrame::Callback* cb) : windowSize(size), mainLoopStarted(false) {
	SetCallback(cb);
...
void ZFrameDXUT::OnWindowSize(const ZInt2& newSize) {
	windowSize = newSize;
	if (callback != nullptr) {
...
void ZFrameDXUT::SetWindowSize(const ZInt2& size) {
	RECT rect;
	::GetWindowRect(hWnd, &rect);
	ZInt2 center((rect.left + rect.right) / 2, (rect.top + rect.bottom) / 2);
	::SetWindowPos(hWnd, nullptr, center.x() - size.x() / 2, center.y() - size.y() / 2, size.x(), size.y(), 0);
	windowSize = size;
}

const ZInt2& ZFrameDXUT::GetWindowSize() const {
	return windowSize;
}
...
void ZFrameDXUT::WarpCursor(const ZInt2& position) {
