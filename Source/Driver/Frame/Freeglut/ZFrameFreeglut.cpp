...
static ZInt2 MakeScreenPos(ZFrameFreeglut* p, int x, int y) {
	return ZInt2(x, p->GetWindowSize()[1] - y);
}
...
static void MouseClickMessage(int button, int state, int x, int y) {
	...
	if (button == GLUT_LEFT_BUTTON || button == GLUT_RIGHT_BUTTON) {
		p->OnMouse(IFrame::EventMouse(state == 0, false, button == GLUT_LEFT_BUTTON, false, MakeScreenPos(p, x, y)));
	} else {
...
static void MouseMoveMessage(int x, int y) {
	...
	ZFrameFreeglut* p = mapWindows[nWindow];
	assert(p != nullptr);
	p->OnMouse(IFrame::EventMouse(true, true, true, false, MakeScreenPos(p, x, y)));
}
...
static void MousePassiveMoveMessage(int x, int y) {
	...
	ZFrameFreeglut* p = mapWindows[nWindow];
	assert(p != nullptr);
	p->OnMouse(IFrame::EventMouse(false, true, true, false, MakeScreenPos(p, x, y)));
}
...
static void MouseWheelMessage(int wheel, int dir, int x, int y) {
	...
	ZFrameFreeglut* p = mapWindows[nWindow];
	assert(p != nullptr);
	p->OnMouse(IFrame::EventMouse(dir > 0, false, false, true, MakeScreenPos(p, x, y)));
}
...
static void SpecialKeys(int key, int x, int y) {
	...
	if ((key & IFrame::EventKeyboard::KEY_CTRL) == IFrame::EventKeyboard::KEY_CTRL
		...
		p->OnKeyboard(IFrame::EventKeyboard(key));
	} else {
		p->OnKeyboard(IFrame::EventKeyboard(key));
	}
}
...
static void KeyboardFunc(unsigned char cAscii, int x, int y) {
	...
	p->OnKeyboard(IFrame::EventKeyboard(AddKeyModifier((int)cAscii)));
}

static void KeyboardFuncUp(unsigned char cAscii, int x, int y) {
	int nWindow = glutGetWindow();
	ZFrameFreeglut* p = mapWindows[nWindow];
	p->OnKeyboard(IFrame::EventKeyboard(AddKeyModifier((int)cAscii)| IFrame::EventKeyboard::KEY_POP));
}

static void ChangeSize(int w, int h) {
	int nWindow = glutGetWindow(); // Get current window
	ZFrameFreeglut* p = mapWindows[nWindow]; // Find the instance

	p->OnWindowSize(ZInt2(w, h)); // Dispatch
}
...
ZFrameFreeglut::ZFrameFreeglut(const ZInt2& size, IFrame::Callback* cb) : windowSize(size), mainLoopStarted(false), isRendering(false) {
	SetCallback(cb);
...
void ZFrameFreeglut::OnWindowSize(const ZInt2& newSize) {
	if (callback != nullptr) {
...
void ZFrameFreeglut::SetWindowSize(const ZInt2& size) {
	glutReshapeWindow(size.x(), size.y());
	windowSize = size;
}

const ZInt2& ZFrameFreeglut::GetWindowSize() const {
	return windowSize;
}
...
void ZFrameFreeglut::WarpCursor(const ZInt2& position) {
