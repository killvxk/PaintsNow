...
void Config::RegisterFactory(const String& factoryEntry, const String& name, const TWrapper<void*, const String&>* factoryBase) {
	...
	mapFactories[factoryEntry].push_back(std::move(entry));
...
void Config::RegisterRuntimeHook(const TWrapper<void, NsLeavesWind::LeavesWind*, RUNTIME_STATE>& proc) {
	runtimeHooks.push_back(proc);
}
...
void Config::PostRuntimeState(NsLeavesWind::LeavesWind* leavesWind, RUNTIME_STATE state) {
	for (size_t i = 0; i < runtimeHooks.size(); i++) {
		runtimeHooks[i](leavesWind, state);
	}
}