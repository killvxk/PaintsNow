...
local QuickCompile = {}
EnableTypedlua = true
local UtilModule, errMsg = load(System.SnowyStream.QuickTextResource("Engine/Boot/Util.lua"))
if errMsg then
...
function QuickCompile:CompileOne(filename, outputfile)
	...
	-- read file content
	local content = SnowyStream.QuickTextResource(filename)
	if content then
		local code, err = typedlua.compile(content, title, false, { STRICT = true, INTEGER = true, SHOW_WARNINGS = false})
		if code then
			local t = SnowyStream.CreateFile(outputfile, true)
			if t then
...
function QuickCompile:BuildUpdatedOne(filename, tree, forceRecompile)
	...
	if not forceRecompile then
		local source = SnowyStream.CreateFile(filename, false)
		if source then
			local checker = SnowyStream.CreateFile(outputfile, false)
			if checker then
				local deps = false
...
local function Main(inputFileName)
	if inputFileName then
		QuickCompile:CompileRecursive(inputFileName, true)
	end
end
