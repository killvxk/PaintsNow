			...
			for (DISPID i = 0; i < attr->cFuncs; i++) {
				...
				if (SUCCEEDED(info->GetFuncDesc(i, &func)) && func->memid < 0x10000000) {
					...
					for (SHORT i = 0; i < func->cParams; i++) {
						...
						method.params.push_back(action);
						method.parsers.push_back(static_cast<const AutoVariantHandler*>(reflectMap[action.type].parser));
					}
...
TObject<IReflect>& ComDispatch::operator () (IReflect& reflect) {
	for (size_t i = 0; i < methods.size(); i++) {
		...
		reflect.ProcessMethod(Unique(), m.name.c_str(), reinterpret_cast<TProxy<>*>(i), m.retValue, m.params, nullptr);
	}
	return *this;
}
