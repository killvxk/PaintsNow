...
static uint8_t GetSize(const PodList* p) {
	if (p == NULL) return sizeof(uint8_t);
		...
		if (p->offset != (PodSize)-1) {
			...
			if (PodIsPlain(p->node)) {
				...
				if (count != 0) {
					...
					if (pos != NULL) {
						if (t->subList == NULL || (t->iterateHandler != NULL && !t->subList->isDynamic) || (PodIsPlain(t->subList->node) && (IsStdEndian() || !t->subList->isInteger))) {
							/* accelerate plain data */
							if ((status = ReadStream(stream, (char*)pos + (t->subList == NULL ? 0 : t->subList->offset), count * GetSize(t->subList), 0, context)) != POD_SUCCESS)
								return status;
						} else {
							// assert(count == 1);
			...
			if (PodIsPlain(p->node)) {
				...
				if (count != 0) {
					if (pos != NULL) {
						if (t->subList == NULL || (t->iterateHandler != NULL && !t->subList->isDynamic) || (PodIsPlain(t->subList->node) && (IsStdEndian() || !t->subList->isInteger))) {
							/* accelerate plain data */
							if ((status = WriteStream(stream, (char*)pos + (t->subList == NULL ? 0 : t->subList->offset), count * GetSize(t->subList), 0, context)) != POD_SUCCESS)
								return status;
						} else {
							assert(count == 1);
