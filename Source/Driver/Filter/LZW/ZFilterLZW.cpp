...
inline void WriteBuffer(std::vector<unsigned char>& buffer, uint32_t value) {
	...
	if (value > 0xF0000000) {
		...
		buffer.push_back(0xE0 | (value >> 24));
		from = 2;
	} else if (value > 0xC000) {
		buffer.push_back(0xC0 | (value >> 16));
		from = 1;
	} else if (value >= 0x80) {
		buffer.push_back(0x80 | (value >> 8));
		from = 0;
	}
...
bool FilterLZWImpl::Write(IReflectObject& s, void* ptr, size_t length) {
	...
	if (reflectWriter.data != nullptr) {
		uint32_t size = (uint32_t)reflectWriter.data->size();
		stream << size;
		size_t s = size;
