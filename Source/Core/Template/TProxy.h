...
// #define COMPATIBLE_PROXY

#if defined(_MSC_VER) && _MSC_VER >= 1800 && defined(USE_STD_FUNCTION)
#include <functional>
#endif
...
// remove_reference for vc6, from boost library
...
namespace std {
	...
	template <typename T>
	struct is_reference {
		enum { value = detail::is_reference_impl<T>::value };
	};
	...
	namespace detail {
		...
		struct msvc_register_type : msvc_extract_type<ID> {
			...
			struct base_type::id2type { typedef T type; };
		};
	...
	struct remove_reference {
		typedef typename detail::remove_reference_impl_typeof<std::is_reference<T>::value>::template inner<T,remove_reference<T> >::type type;
	};
}
#endif
