...
bool ZPacket::WritePacket(int64_t seq, IStreamBase& source, uint32_t length) {
	...
	int padding = (uint32_t)FullLength(location, fullLength);
...
//	int64_t savedLocation = location;
	...
	int64_t remaining = alignedEnd;
 	...
 	while (length > 0) {
		uint32_t alignment = (uint32_t)(location < mid ? Alignment(location | mid) : AlignmentTop(remaining - mid));
		...
		uint32_t len = (uint32_t)fullLength;
		wl = sizeof(len);
	...
	do {
		...
		if (length == 0) {
			...
			if (!stream.Read(&length, rl)) {
				...
				length -= (uint32_t)header.padding * sizeof(PacketHeader);
